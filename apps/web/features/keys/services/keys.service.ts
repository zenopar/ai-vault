...
export async function listApiKeysService(): Promise<AiApiKeyMetadata[]> {
  const response = await fetchWithSession((sessionToken) =>
    VaultApiClient.sendGetRequest<ListApiKeysResponse>("/keys", { sessionToken })
  );
...
export async function addApiKeyService(params: {
  ...
  const response = await fetchWithSession((sessionToken) =>
    VaultApiClient.sendPostRequest<AddApiKeyResponse>("/keys", {
...
export async function deleteApiKeyService(id: string): Promise<boolean> {
  const response = await fetchWithSession((sessionToken) =>
    VaultApiClient.sendDeleteRequest<{ success: boolean }>(`/keys/${id}`, {
      sessionToken,
    })
  );
...
export async function addModelService(params: {
  ...
  const response = await fetchWithSession((sessionToken) =>
    VaultApiClient.sendPostRequest<{ success: boolean; model?: AiModelMetadata; error?: string }>("/models", {
      provider: params.provider,
      name: params.name,
...
export async function deleteModelService(id: string): Promise<boolean> {
  const response = await fetchWithSession((sessionToken) =>
    VaultApiClient.sendDeleteRequest<{ success: boolean }>(`/models/${id}`, {
      sessionToken,
    })
  );
